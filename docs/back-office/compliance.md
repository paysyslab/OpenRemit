...
export const ImgCard = ({ src, alt, label }) => {
  ...
  const styles = useMemo(() => ({
    card: { background: '#fff', border: '1.5px solid #e2e8f0', borderRadius: 14, overflow: 'hidden', cursor: 'zoom-in', display: 'flex', flexDirection: 'column' },
...
export const Compliance = () => {
  const s = useMemo(() => {
    ...
    return {
      ...
      card: { background: '#fff', border, borderRadius: 14, padding: '24px 28px', display: 'flex', gap: 20, alignItems: 'flex-start' },
      iconWrap: { flexShrink: 0, width: 46, height: 46, borderRadius: 12, background: '#e8f2fa', color: '#1E6FA8', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 },
      stepBody: { flex: 1 },
